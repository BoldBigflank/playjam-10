...
--  https://github.com/macvogelsang/pd-options
--
import 'CoreLibs/ui'
...
function Options:init(definitions, displayOnRight, saveDataPath, onHide)
    ...
    function self.menu.drawCell(menuSelf, section, row, column, selected, x, y, width, height)
        ...
        if val ~= 'n/a' and val ~= nil then
            ...
            elseif style ~= RESET then
                ...
                gfx.drawTextInRect(val, labelWidth + textPadding, y + textPadding, optionWidth, height, nil, '...',
    ...
    self.controls = {
        ...
        leftButtonDown = function()
            self.keyTimer.L = timer.keyRepeatTimerWithDelay(LEFT_RIGHT_KEY_REPEAT_INITIAL, LEFT_RIGHT_KEY_REPEAT,
                function() self:toggleCurrentOption(-1) end)
        end,
        leftButtonUp = function() if self.keyTimer.L then self.keyTimer.L:remove() end end,
        rightButtonDown = function()
            self.keyTimer.R = timer.keyRepeatTimerWithDelay(LEFT_RIGHT_KEY_REPEAT_INITIAL, LEFT_RIGHT_KEY_REPEAT,
                function() self:toggleCurrentOption(1) end)
        end,
        rightButtonUp = function() if self.keyTimer.R then self.keyTimer.R:remove() end end,
        upButtonDown = function()
            self.keyTimer.U = timer.keyRepeatTimerWithDelay(UP_DOWN_KEY_REPEAT_INITIAL, UP_DOWN_KEY_REPEAT,
                function() self:selectPreviousRow() end)
        end,
        upButtonUp = function() if self.keyTimer.U then self.keyTimer.U:remove() end end,
        downButtonDown = function()
            self.keyTimer.D = timer.keyRepeatTimerWithDelay(UP_DOWN_KEY_REPEAT_INITIAL, UP_DOWN_KEY_REPEAT,
                function() self:selectNextRow() end)
        end,
